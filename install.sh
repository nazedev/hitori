...
run_with_spinner() {
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        printf "  ${GREEN}✔${NC} %s ${GRAY}[${NC}%b${GRAY}]${NC} ${DIM}100%%${NC}\n" "$msg" "$full_bar"
    else
        local full_bar="${RED}${full_bar_solid}${NC}"
        printf "  ${RED}✖${NC} %s ${GRAY}[${NC}%b${GRAY}]${NC} ${RED}FAIL${NC}\n" "$msg" "$full_bar"
        echo -e "\n  ${RED}=== ERROR LOG ===${NC}"
...
main() {
    ...
    # Register global 'hitori' command
    local link_cmd=""
    if [ "$PKG" = "pkg" ]; then
    ...
    run_with_spinner "Registering global 'hitori' command" "$link_cmd"
