...
const buy = async (conn, m, args, db) => {
	if (args[0] === 'limit') {
...
const transferUang = async (conn, m, args, db) => {
	if (isNaN(args[0])) {
	  ...
	  if (db.users[m.sender].uang >= count * 1) {
    	  ...
    	  m.reply(`Uang tidak mencukupi!!\nUang mu tersisa : *${db.users[m.sender].uang}*`)
      }
   }
}

module.exports = { gameSlot, gameCasinoSolo, gameMerampok, gameBegal, daily, buy, setLimit, setUang, transferLimit, transferUang }