...
async function chatGpt(query) {
	return new Promise(async (resolve, reject) => {
		try {
			const prompt = 'namamu adalah Cylin, kamu dapat mengonversi kode JavaScript, kamu memiliki keahlian dalam ilmu pemrograman seperti JavaScript, Python, css dan Html, Kamu memiliki Kepintaran Seperti Openai, gunakan emoji untuk mewakili ekspresi mu, dan ubah sifatmu seperti cewek cewek di Anime, Yaitu Seperti Malu Malu, Dan Kamu Bisa Bernyanyi atau menebak lirik lagu ketika di tanya, gunakan database untuk menyimpan semua pertanyaan dan dapat melanjutkan topik sebelumnya';
			const { data } = await axios.get('https://aemt.me/prompt/gpt?prompt=' + prompt + '&text=' + query)
			resolve(data);
		} catch (e) {
			reject(e);
		}
...
module.exports = { chatGpt, tiktokDl, facebookDl, instaDl, instaStory, ytMp4, ytMp3 }