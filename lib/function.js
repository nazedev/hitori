...
const extractUrl = (text, priorityDomain) => {
	...
	const ignorePatterns = [
		...
		/whatsapp\.com\/dl/i,
		/shopee\.co\.id\/universal-link/i
	];

	const validUrls = matches
		.map((u) => u.replace(/[.,;?!)]+$/, ''))
		.filter((url) => !ignorePatterns.some((pattern) => pattern.test(url)));
...
const isUrl = (url) => {
	...
	const validUrls = matches
		.map((u) => u.replace(/[.,;?!)]+$/, ''))
		.filter((u) => !ignorePatterns.some((pattern) => pattern.test(u)));
	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (setLimitRole) {
				...
				const updated = body.replace(new RegExp(`(${escapedRole}\\s*:\\s*)\\d+`), `$1${value}`);
...
function assertInstalled(cmd, name, code) {
	try {
		...
		execFileSync(parts[0], parts.slice(1), { stdio: 'ignore' });
	} catch (e) {
...
function tarBackup(source, output) {
	return new Promise((resolve, reject) => {
		execFile('tar', ['-czf', output, `--exclude=${output}`, '--exclude=./node_modules', source], (err, stdout, stderr) => {
...
const compressMessage = (msg) => {
	...
	try {
		...
		const cleanObj = proto.WebMessageInfo.toObject(
