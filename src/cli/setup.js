 ...
 * Walks the user through essential configuration on first install.
 * Writes values directly into settings.js via the settings parser.
 */
...
export async function runSetupWizard() {
	...
	const ownerRaw = await input({
		...
		validate: (val) => {
			...
			for (const n of nums) {
				// Strip any + prefix if user provided it, then prepend + for parsing
	...
	// ── 4. Naze API Key ────────────────────────────────────────────────
	logInfo("Naze API Key — get yours at https://sock.biz.id");
	...
	const botNumberRaw = await input({
		...
		validate: (val) => {
			if (!val.trim()) return true; // allowed to be empty
	...
	console.log(`  ${theme.dim("Tip: Run")} ${theme.accent("hitori settings")} ${theme.dim("to change more options anytime.")}\n`);
