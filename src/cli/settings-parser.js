 ...
 * Reads and writes values directly in settings.js without destroying
 * comments, formatting, or unrelated lines.
 *
 * Strategy:
 *   - Parse known global.* assignments via regex
 *   - When writing, do a targeted line replacement (only the matched line)
 *   - Unknown/complex values are left untouched
 */

// ── Mapping of config keys → regex + serializer ──────────────────────
const FIELD_MAP = {
	owner: {
	...
	nazeApiKey: {
		// Matches the line:  'https://api.naze.biz.id': 'some-key',
		pattern: /^(\s*['"]https:\/\/api\.naze\.biz\.id['"]\s*:\s*)(['"].*?['"])/,
		serialize: (val) => JSON.stringify(val),
	},
};

// ── Read a single value from settings.js ─────────────────────────────
export function readSetting(key) {
	...
	// For limit fields, we need to find them inside the global.limit block
	...
	for (const line of lines) {
		...
		if (inSection) {
			...
			if (match) {
				...
				let result;
				try {
					// Try JSON parse first (handles arrays, quoted strings)
					result = JSON.parse(raw);
				} catch {
					try {
						// Fallback: evaluate JS literal (handles single quotes, trailing commas, etc.)
						result = new Function(`return ${raw};`)();
					} catch {
						// Final fallback: strip quotes
...
// ── Write a single value to settings.js (in-place line replacement) ──
export function writeSetting(key, value) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (inSection) {
			...
			if (match) {
				// Replace only the value portion, keep prefix (indentation + key) and suffix (comments)
				const prefix = match[1];
				const oldValue = match[2];
				const newValue = field.serialize(value);

				// Find exact position of the old value in the line
...
export function writeSettings(config) {
	// Read file once, apply all changes, write once
	const content = fs.readFileSync(SETTINGS_FILE, "utf-8");
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (inSection) {
				...
				if (match) {
					...
					lines[i] = before + newValue + after;
					break; // Found and replaced for this key
				}
			}
		}
	}
...
// "?"? Special parser for global.database multi-line object "?"?
...
// ── Check if first-run (owner is still default placeholder) ──────────
