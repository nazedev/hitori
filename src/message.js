...
async function Solving(sock, store) {
	...
	sock.sendFromOwner = async (jids, text, quoted, options = {}) => {
		for (const a of jids) {
			...
			await sock.sendMessage(jid, { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		}
	};
	...
	sock.sendText = async (jid, text, quoted, options = {}) =>
		sock.sendMessage(
			...
			{ quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 },
		);
