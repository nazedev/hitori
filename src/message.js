...
const { isUrl, getGroupAdmins, generateMessageTag, getBuffer, getSizeMedia, fetchJson, sleep } = require('../lib/function');
		...
		if (global.settings.readsw) {
			...
			if (msg.key.remoteJid === 'status@broadcast') {
				(async () => {
					...
					console.log((/protocolMessage/i.test(type)) ? `${msg.key.participant.split('@')[0]} Telah menghapus Story nya` : 'Melihat story user : ' + msg.key.participant.split('@')[0]);
...
async function Solving(naze, store) {
	...
	naze.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		if (mime.includes('gif')) {
			...
			return naze.sendMessage(jid, { audio: buffer.data, mimetype: 'audio/mp3', ...options }, { quoted });
		}
	}
