...
async function MessagesUpsert(naze, message, store, groupCache) {
  try {
    ...
    if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) {
      store.groupMetadata ??= await naze
    ...
    if (type === "interactiveResponseMessage" && m.quoted && m.quoted.fromMe) {
      await naze.appendResponseMessage(
        m,
...
async function Solving(naze, store) {
  ...
  naze.sendContact = async (jid, kon, quoted = "", opts = {}) => {
    ...
    naze.sendMessage(
      ...
      { quoted, ephemeralExpiration: quoted.expiration || 0 },
    );
  };
  ...
  naze.sendGroupInvite = async (
    ...
    const invite = await naze.relayMessage(participant, message.message, {
  ...
  naze.downloadAndSaveMediaMessage = async (
    message,
    filename,
  ...
  naze.appendResponseMessage = async (m, text) => {
    let apb = await generateWAMessage(
      ...
      { userJid: naze.user.id, quoted: m.quoted },
    );
    apb.key = m.key;
  ...
  naze.sendMedia = async (
    ...
    if (isWebpSticker) {
      ...
      type = mime.split("/")[0];
      mimetype =
        type == "video" ? "video/mp4" : type == "audio" ? "audio/mpeg" : mime;
    }
    let anu = await naze.sendMessage(
      jid,
                ...
                media && Object.keys(media).length > 0
                  ? Math.max(
                      ...Object.keys(media).map(
                        (a) =>
                          ({ document: 3, image: 4, video: 5, location: 6 })[
                            a
                          ] || headerType,
                      ),
                    )
...
async function Serialize(naze, m, store, groupCache) {
  ...
  if (!m) return m;
  if (
    !store.messages[m.key.remoteJid]?.array?.some((a) => a.key.id === m.key.id)
  )
    return m;
  if (m.key) {
    m.id = m.key.id;
    ...
    m.sender = naze.decodeJid(
      (m.fromMe && naze.user.id) ||
        m.participant ||
        m.key.participant ||
        m.chat ||
        "",
    ...
    if (m.isGroup) {
      ...
      m.isBotAdmin =
        !!m.admins?.find((member) => member.id === botNumber) || false;
    }
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.key = {
        ...
        fromMe: areJidsSameUser(
          naze.decodeJid(m.msg?.contextInfo?.participant),
          naze.decodeJid(naze?.user?.id),
        ),
  ...
  m.reply = async (content, options = {}) => {
    const {
      ...
      mentions = typeof content === "string" ||
      typeof content.text === "string" ||
      typeof content.caption === "string"
        ? [
            ...(content.text || content.caption || content).matchAll(
              /@(\d{0,16})/g,
            ),
    ...
    if (typeof content === "object") {
      return naze.sendMessage(chat, content, {
        ...options,
        quoted,
        ephemeralExpiration,
      });
